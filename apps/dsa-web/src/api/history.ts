...
export const historyApi = {
  ...
  /**
   * 获取历史报告详情
   * @param recordId Analysis history primary key or async task query_id
   */
  getDetail: async (recordId: number | string): Promise<AnalysisReport> => {
    const response = await apiClient.get<Record<string, unknown>>(`/api/v1/history/${encodeURIComponent(String(recordId))}`);
