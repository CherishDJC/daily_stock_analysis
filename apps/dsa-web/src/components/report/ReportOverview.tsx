...
export const ReportOverview: React.FC<ReportOverviewProps> = ({
  ...
  const buildFallbackInsight = (): string => {
    const parts: string[] = [];
    if (summary.operationAdvice) parts.push(`操作建议：${summary.operationAdvice}`);
    if (summary.trendPrediction) parts.push(`趋势判断：${summary.trendPrediction}`);
    if (meta.currentPrice != null) parts.push(`快照价格：${meta.currentPrice.toFixed(2)} 元`);
    return parts.length > 0 ? parts.join('；') : '暂无分析结论';
  };

  const keyInsights = summary.analysisSummary?.trim() || buildFallbackInsight();
