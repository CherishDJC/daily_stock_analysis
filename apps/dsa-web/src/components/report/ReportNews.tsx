  ...
  return (
    <Card variant="bordered" padding="md">
      ...
      {!isLoading && !error && items.length === 0 && (
        <div className="rounded-lg border border-white/8 bg-elevated/60 p-3 text-left">
          <p className="text-xs text-white">当前报告没有采集到相关资讯</p>
          <p className="mt-1 text-xs text-muted">
            系统会自动尝试免 Key 的
            {' '}
            <span className="font-mono text-cyan">GDELT</span>
            {' '}
            公开新闻源；如果你需要更完整的中文财经新闻，可额外配置
            {' '}
            <span className="font-mono text-cyan">TAVILY_API_KEYS</span>
            {' / '}
            <span className="font-mono text-cyan">BOCHA_API_KEYS</span>
            {' / '}
            <span className="font-mono text-cyan">BRAVE_API_KEYS</span>
            {' / '}
            <span className="font-mono text-cyan">SERPAPI_API_KEYS</span>
            作为增强搜索源。
          </p>
        </div>
      )}
