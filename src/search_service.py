...
职责：
1. 提供统一的新闻搜索接口
2. 支持 Bocha、Tavily、Brave、SerpAPI 和 GDELT 搜索引擎
3. 多 Key 负载均衡和故障转移
4. 搜索结果缓存和格式化
"""
...
from src.news_text import sanitize_news_snippet, sanitize_news_title
...
def sanitize_search_result(result: SearchResult) -> SearchResult:
    """Normalize provider output before it is consumed or persisted."""
    title_fallback = result.source or result.url or "未命名资讯"
    return SearchResult(
        title=sanitize_news_title(result.title, fallback=title_fallback),
        snippet=sanitize_news_snippet(result.snippet),
        url=(result.url or "").strip(),
        source=(result.source or "").strip() or "未知来源",
    ...
    def search(self, query: str, max_results: int = 5, days: int = 7) -> SearchResponse:
        ...
        try:
            ...
            if response.success and response.results:
                response.results = [sanitize_search_result(item) for item in response.results]
...
class GDELTSearchProvider(BaseSearchProvider):
    """
    GDELT DOC 2.0 public search provider.
    ...
    def __init__(self):
        super().__init__([self._PUBLIC_KEY], "GDELT")
    ...
    def _request_with_rate_limit(
        ...
        *,
        ...
        with cls._rate_limit_lock:
            wait_seconds = 0.0
            if force_delay:
                wait_seconds = cls._MIN_REQUEST_INTERVAL_SECONDS
            else:
                elapsed = time.monotonic() - cls._last_request_ts
                wait_seconds = max(0.0, cls._MIN_REQUEST_INTERVAL_SECONDS - elapsed)

            if wait_seconds > 0:
                logger.info(f"[GDELT] Waiting {wait_seconds:.1f}s to respect public API rate limit")
                time.sleep(wait_seconds)

            cls._last_request_ts = time.monotonic()

        return _get_with_retry(cls.API_ENDPOINT, params=params, timeout=10)
    ...
    def _do_search(self, query: str, api_key: str, max_results: int, days: int = 7) -> SearchResponse:
        ...
        try:
            ...
            response = self._request_with_rate_limit(params)
            if response.status_code == 429:
                logger.warning("[GDELT] Hit public API rate limit, retrying once after a cooldown")
                response = self._request_with_rate_limit(params, force_delay=True)
            if response.status_code != 200:
                return SearchResponse(
                    query=query,
                    results=[],
            ...
            try:
                ...
                return SearchResponse(
                    ...
                    provider=self.name,
                    success=False,
                    error_message=f"响应JSON解析失败: {exc}",
                )
            ...
            for item in raw_articles[:max_results]:
                ...
                published_date = self._format_seen_date(item.get("seendate") or item.get("seenDate"))
                snippet = self._build_snippet(item, source)
                results.append(
                    SearchResult(
                        title=item.get("title", ""),
                        snippet=snippet,
            ...
            logger.info(f"[GDELT] 搜索完成，query='{query}', gdelt_query='{gdelt_query}', 返回 {len(results)} 条结果")
            logger.debug(f"[GDELT] 原始响应 keys: {list(data.keys())}")
    ...
    def _build_query(cls, query: str) -> str:
        ...
        if not normalized:
            return query
        if any(marker in normalized for marker in ('(', ')', '"', ':', ' OR ', ' AND ')):
            return normalized
        ...
        code_index = next((idx for idx, token in enumerate(tokens) if cls._looks_like_symbol(token)), None)
        if code_index is not None:
            name_term = " ".join(tokens[:code_index]).strip()
            code_term = tokens[code_index]
            context_tokens = tokens[code_index + 1:]
        else:
            name_term = tokens[0]
            code_term = None
        ...
        identity_terms = []
        if name_term:
            identity_terms.append(name_term)
        if code_term:
            identity_terms.append(code_term)

        context_terms = [
            token for token in context_tokens
            if token and token.lower() not in cls._GENERIC_QUERY_TERMS and token not in cls._GENERIC_QUERY_TERMS
        ][:3]

        identity_query = " OR ".join(f'"{term}"' for term in identity_terms if term)
        context_query = " OR ".join(f'"{term}"' for term in context_terms if term)

        if identity_query and context_query:
            return f"({identity_query}) AND ({context_query})"
        if identity_query:
            return f"({identity_query})"
        if context_query:
            return f"({context_query})"
        return normalized
    ...
    def _looks_like_symbol(token: str) -> bool:
        ...
        if not normalized_token:
            return False
        if re.fullmatch(r"\d{5,6}", normalized_token):
            return True
        if normalized_token.startswith("HK") and re.fullmatch(r"HK\d{4,5}", normalized_token):
            return True
        return raw_token == normalized_token and re.fullmatch(r"[A-Z]{1,5}(?:\.[A-Z])?", normalized_token) is not None

    @staticmethod
    def _format_seen_date(value: Optional[str]) -> Optional[str]:
        if not value:
            return None
        try:
    ...
    @staticmethod
    def _build_snippet(item: Dict[str, Any], source: str) -> str:
        parts = [f"来源: {source}"]
        ...
        if source_country:
            parts.append(f"地区: {source_country}")
        return " | ".join(parts)
...
class SearchService:
    ...
    def __init__(
        ...
        # 5. GDELT public fallback (no API key required)
        self._providers.append(GDELTSearchProvider())
        logger.info("已启用 GDELT 公开新闻搜索（免 API Key）")

        if not any([bocha_keys, tavily_keys, brave_keys, serpapi_keys]):
            logger.info("未配置搜索引擎 API Key，将回退到 GDELT 公开新闻源")
    ...
    def provider_names(self) -> List[str]:
        """Return available provider names for logging and diagnostics."""
