...
class HistoryService:
    ...
    def _build_analysis_summary(
        ...
        *,
        ...
        if isinstance(record.analysis_summary, str) and record.analysis_summary.strip():
            return record.analysis_summary.strip()

        if isinstance(raw_result, dict):
            raw_summary = raw_result.get("analysis_summary")
            ...
            if isinstance(dashboard, dict):
                ...
                if isinstance(core_conclusion, dict):
                    ...
                    if isinstance(one_sentence, str) and one_sentence.strip():
                        return one_sentence.strip()

                provider_error = self._extract_provider_error(raw_result)
                if provider_error:
                    return f"AI 分析失败：{provider_error}"

        fallback_parts: List[str] = []
        if record.operation_advice:
            fallback_parts.append(f"操作建议：{record.operation_advice}")
        if record.trend_prediction:
            fallback_parts.append(f"趋势判断：{record.trend_prediction}")

        if isinstance(context_snapshot, dict):
            realtime = context_snapshot.get("realtime_quote") or {}
            price = realtime.get("price")
            if price is not None:
                try:
                    fallback_parts.append(f"快照价格：{float(price):.2f} 元")
                except (TypeError, ValueError):
                    fallback_parts.append(f"快照价格：{price}")

        if fallback_parts:
            return "；".join(fallback_parts)

        return "当前记录未生成可展示的分析摘要"

    @staticmethod
    def _extract_provider_error(raw_result: Dict[str, Any]) -> Optional[str]:
        dashboard = raw_result.get("dashboard")
        candidates = [dashboard, raw_result.get("error"), raw_result]
        ...
        for candidate in candidates:
            if isinstance(candidate, dict):
                ...
                if isinstance(message, str) and message.strip():
                    ...
                    if code_text and code_text != raw_result.get("code"):
    ...
    def get_news_intel(self, query_id: str, limit: int = 20) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for record in records:
                title = sanitize_news_title(record.title)
                snippet = sanitize_news_snippet(record.snippet)
                if len(snippet) > 200:
