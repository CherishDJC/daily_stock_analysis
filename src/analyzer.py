...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def _call_openai_api(self, prompt: str, generation_config: dict) -> str:
        ...
        max_retries = config.gemini_max_retries
        base_delay = config.gemini_retry_delay
        api_style = normalize_openai_api_style(getattr(config, "openai_api_style", None))

        if api_style == "responses":
            return self._call_openai_responses_api(prompt, generation_config, max_retries, base_delay)
        ...
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    delay = base_delay * (2 ** (attempt - 1))
                    delay = min(delay, 60)
                    logger.info(f"[OpenAI/Responses] 第 {attempt + 1} 次重试，等待 {delay:.1f} 秒...")
                    time.sleep(delay)
                ...
                if text:
                    return text
                raise ValueError("OpenAI Responses API 返回空响应")
                ...
                if is_rate_limit:
                    logger.warning(f"[OpenAI/Responses] API 限流，第 {attempt + 1}/{max_retries} 次尝试: {error_str[:100]}")
                else:
                    logger.warning(f"[OpenAI/Responses] API 调用失败，第 {attempt + 1}/{max_retries} 次尝试: {error_str[:100]}")

                if attempt == max_retries - 1:
                    raise

        raise Exception("OpenAI Responses API 调用失败，已达最大重试次数")
...
请输出完整的 JSON 格式决策仪表盘。"""
    ...
    def _parse_response(
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                provider_error = self._extract_provider_error(data, code)
                if provider_error:
                    logger.warning(f"AI provider returned an error payload for {code}: {provider_error}")
                    return AnalysisResult(
                        code=code,
                        name=name,
                        sentiment_score=50,
                        ...
                        decision_type='hold',
                        confidence_level='低',
                        analysis_summary=f'AI 分析失败：{provider_error}',
                        risk_warning='请检查 API Key、网关配置或模型可用性后重试',
    ...
    @staticmethod
    def _extract_provider_error(data: Dict[str, Any], stock_code: str) -> Optional[str]:
        """Detect common upstream error payloads that were wrapped as JSON responses."""
        if not isinstance(data, dict):
            return None

        dashboard = data.get('dashboard')
        dashboard_error = GeminiAnalyzer._extract_error_message_from_candidate(
            dashboard,
            expected_code=stock_code,
            allow_generic_invalid_structure=True,
        )
        if dashboard_error:
            return dashboard_error

        top_level_error = GeminiAnalyzer._extract_error_message_from_candidate(
            data,
            expected_code=stock_code,
            allow_generic_invalid_structure=False,
        )
        if top_level_error:
            return top_level_error

        if GeminiAnalyzer._looks_like_invalid_dashboard_payload(dashboard):
            return 'AI 返回了无效响应，请检查 API 网关或鉴权配置'

        return None

    @staticmethod
    def _extract_error_message_from_candidate(
        candidate: Any,
        *,
        expected_code: Optional[str],
        allow_generic_invalid_structure: bool,
    ) -> Optional[str]:
        if not isinstance(candidate, dict):
            if isinstance(candidate, str):
                stripped = candidate.strip()
                if stripped and ('invalid api key' in stripped.lower() or 'unauthorized' in stripped.lower()):
                    return stripped
            return None
        ...
        if message_text:
            known_dashboard_keys = {'core_conclusion', 'data_perspective', 'battle_plan', 'intelligence', 'risk_flags'}
            if allow_generic_invalid_structure and not known_dashboard_keys.intersection(candidate.keys()):
                if code_text and expected_code and code_text == expected_code:
                    return None
                return f"{code_text}: {message_text}" if code_text else message_text
            if code_text and expected_code and code_text == expected_code:
                return None
            if code_text and code_text.upper() != code_text and code_text.isdigit():
                return None
            return f"{code_text}: {message_text}" if code_text else message_text
    ...
    @staticmethod
    def _looks_like_invalid_dashboard_payload(dashboard: Any) -> bool:
        if not isinstance(dashboard, dict) or not dashboard:
            return False

        expected_keys = {'core_conclusion', 'data_perspective', 'battle_plan', 'intelligence', 'risk_flags'}
        if expected_keys.intersection(dashboard.keys()):
            return False

        suspicious_markers = ('<html', 'cf-', 'classlist', 'addEventListener', 'hidden')
        serialized = json.dumps(dashboard, ensure_ascii=False)
        serialized_lower = serialized.lower()
        return any(marker.lower() in serialized_lower for marker in suspicious_markers)
