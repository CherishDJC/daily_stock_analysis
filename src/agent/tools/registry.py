...
class ToolDefinition:
    ...
    def to_openai_responses_tool(self) -> dict:
        """Convert to OpenAI Responses API ``tools`` list element format."""
        return {
            "type": "function",
            "name": self.name,
            "description": self.description,
...
class ToolRegistry:
    ...
    def to_openai_responses_tools(self) -> List[dict]:
        """Generate OpenAI Responses API tools list."""
        return [t.to_openai_responses_tool() for t in self._tools.values()]
