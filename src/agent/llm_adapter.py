...
class LLMToolAdapter:
    ...
    def _call_openai(
        ...
        if normalize_openai_api_style(getattr(self._config, "openai_api_style", None)) == "responses":
            return self._call_openai_responses(messages, tools)
    ...
    def _call_openai_responses(
        ...
        if tools:
            call_kwargs["tools"] = build_responses_tools(tools)

        response = self._openai_client.responses.create(**call_kwargs)
        if getattr(response, "error", None):
            raise ValueError(str(response.error))

        text_content = extract_text_from_responses(response)
        raw_tool_calls = extract_tool_calls_from_responses(response)
        tool_calls = [
            ToolCall(
                id=tc["id"],
                name=tc["name"],
        ...
        return LLMResponse(
            ...
            tool_calls=tool_calls,
            usage=extract_usage_from_responses(response),
            provider="openai",
            raw=response,
        )
