...
from src.openai_compat import normalize_openai_api_style
...
def get_env_path() -> Path:
    """Return the active .env path for this process."""
    env_file = os.getenv("ENV_FILE")
    if env_file:
...
def get_env_file_values() -> Dict[str, Optional[str]]:
    """Read raw key/value pairs from the active .env file."""
...
def read_preferred_env(
    key: str,
    env_file_values: Optional[Dict[str, Optional[str]]] = None,
    default: Optional[str] = None,
) -> Optional[str]:
    """Read a config value, preferring explicit .env entries over process env."""
    values = env_file_values or {}
    if key in values:
        return values[key]
...
class Config:
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # 确保环境变量已加载
        setup_env()
        env_file_values = get_env_file_values()
        ...
        bocha_keys_str = read_preferred_env('BOCHA_API_KEYS', env_file_values, '') or ''
        bocha_api_keys = [k.strip() for k in bocha_keys_str.split(',') if k.strip()]
        
        tavily_keys_str = read_preferred_env('TAVILY_API_KEYS', env_file_values, '') or ''
        tavily_api_keys = [k.strip() for k in tavily_keys_str.split(',') if k.strip()]
        
        serpapi_keys_str = read_preferred_env('SERPAPI_API_KEYS', env_file_values, '') or ''
        serpapi_keys = [k.strip() for k in serpapi_keys_str.split(',') if k.strip()]

        brave_keys_str = read_preferred_env('BRAVE_API_KEYS', env_file_values, '') or ''
        ...
        return cls(
            ...
            tushare_token=read_preferred_env('TUSHARE_TOKEN', env_file_values) or None,
            gemini_api_key=read_preferred_env('GEMINI_API_KEY', env_file_values) or None,
            ...
            anthropic_api_key=read_preferred_env('ANTHROPIC_API_KEY', env_file_values) or None,
            ...
            openai_api_key=read_preferred_env('AIHUBMIX_KEY', env_file_values)
    ...
    def validate(self) -> List[str]:
        ...
        if not self.bocha_api_keys and not self.tavily_api_keys and not self.brave_api_keys and not self.serpapi_keys:
            warnings.append("提示：未配置搜索引擎 API Key，将回退到 GDELT 公开新闻源")
