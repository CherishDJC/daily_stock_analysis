...
[**功能特性**](#-功能特性) · [**快速开始**](#-快速开始) · [**启动文档**](start.md) · [**推送效果**](#-推送效果) · [**完整指南**](docs/full-guide.md) · [**常见问题**](docs/FAQ.md) · [**更新日志**](docs/CHANGELOG.md)
...
| **看盘** | **实时看盘** | **A 股聚合看盘页，展示自选股实时行情、指数、市场宽度与板块冷热** |
...
> 新闻搜索说明：未配置任何搜索引擎 Key 时，系统会自动回退到免 Key 的 `GDELT` 公开新闻源，系统内置了公开源限流节流与一次退避重试；若需要更完整的中文财经资讯和摘要能力，建议额外配置 `BOCHA_API_KEYS` 或 `TAVILY_API_KEYS`。对于少数第三方站点返回的错误编码摘要，系统会自动隐藏乱码片段，仅保留可读标题和原文跳转链接。
> 实时行情说明：盘中主链路默认固定为 `AkShare(tencent -> sina)`；`TUSHARE_TOKEN` 不会自动改写看盘主链路，但会优先增强 A 股历史日线、股票基础信息、分红和股东数据等非实时能力。
...
包含完整的配置管理、任务监控、实时看盘和手动分析功能。

### 实时看盘

启动 Web 服务后，访问 `/monitor` 页面即可进入 A 股实时看盘面板。

- **聚合快照**：展示 `STOCK_LIST` 中 A 股自选股的实时行情、涨跌排序、量比、换手率和成交额
- **市场总览**：同时展示上证/深证/创业板/沪深300、市场宽度和板块领涨/领跌
- **首屏体验**：页面进入后先显示看板框架和自选股快照，指数/市场宽度/板块摘要在后台继续补齐，不再阻塞首屏
- **摘要容错**：市场宽度优先回退到 Tushare 日线统计，板块冷热优先回退到 AkShare 行业快照，减少 summary 空白
- **自动刷新**：盘中默认建议 5 秒轮询；页面切到后台会暂停，回到前台后自动补拉最新快照
- **颜色约定**：A 股涨红跌绿，指数、板块和个股表格统一按该规则渲染
- **走势面板**：点击看盘表中的股票，会打开统一的“走势面板”，顶部可在 `分时脉冲 / 日K回看` 两种视图之间切换
- **免费分钟线**：分时视图新增 `GET /api/v1/stocks/{code}/intraday`，当前使用免费 AkShare 分钟接口，支持 `1/5/15/30/60` 分钟周期切换
- **日内分钟缓存**：后端会按股票缓存当日 1 分钟原始 bars；命中缓存时先秒回 DB 中的分时数据，再在后台异步补最新几分钟，避免页面被慢源阻塞
- **逐笔成交补充**：分时视图右侧会尽力显示最近逐笔成交；若源站波动，该区域会降级为空，但主图仍可继续使用
- **实时刷新**：当走势面板停留在分时视图且处于盘中状态时，页面会按节奏自动刷新最新分钟线；切到后台时暂停，回到前台后补拉一次
- **日K回看**：日K 视图仍支持近 30/60/120 天切换，并展示最近 8 根日线明细
- **AI 快捷分析**：走势面板内提供 `AI分析` 按钮，可直接跳转到首页分析页，并自动把当前股票代码填入输入框
- **板块下钻**：点击板块冷热中的任一行业，可打开最多 10 条相关股明细；继续点击相关股可直接切换到对应日 K，关闭日 K 后会回到刚才的行业列表
- **板块明细回源**：板块下钻优先直连新浪行业成分股接口；若本地股票列表缓存缺少行业字段，会自动跳过坏缓存，避免出现“无相关股信息”
- **全屏看图**：日 K 与分时图都支持一键全屏；全屏模式仅保留图表与退出入口，鼠标悬停可查看对应时间、价格和相对上一根涨跌幅
- **日 K 缓存**：`/api/v1/stocks/{code}/history` 会优先读取本地 `stock_daily`；昨天及更早的日线直接走 DB，只对最近小窗口做补拉
- **范围说明**：首版只服务 A 股盘中场景；若 `STOCK_LIST` 中混入港股/美股代码，页面会显式提示已跳过

> 说明：实时看盘复用现有 `STOCK_LIST` 配置，不新增额外环境变量或单独监控列表。

### 参考数据缓存

系统会将部分低频变化的外部参考数据持久化到主数据库，减少重复请求第三方接口：

- `stock_name`：股票名称，默认缓存 30 天
- `base_info`：股票基础信息、分红和前十大股东等，默认缓存 7 天
- `belong_board`：所属板块/行业映射，默认缓存 7 天
- `index_basic`：指数基础信息，默认缓存 30 天
