...
### 修复（#patch）
- 🐛 **看盘页新增免费分钟级走势面板**
  - 新增 `GET /api/v1/stocks/{code}/intraday`，返回免费分钟 K 线与最近逐笔成交，当前使用 AkShare 免费接口实现
  - Web 看盘页点击股票后，抽屉升级为统一“走势面板”，支持 `分时脉冲 / 日K回看` 双视图切换
  - 分时视图支持 `1/5/15/30/60` 分钟周期切换，右侧补充最近逐笔成交；逐笔接口失败时自动降级为空，不阻塞主图
  - 日 K 全屏逻辑复用到分时图，全屏态仍只保留整屏图表与退出入口
  - 分钟线后端改为按股票缓存当日 1 分钟原始数据；命中缓存时先返回已缓存 bars，再在后台异步补最新几分钟，前端分时视图盘中自动轮询刷新
...
- 🐛 **相关资讯自动隐藏乱码摘要**
  ...
  - 历史报告读取新闻时也会做同样处理，旧记录刷新后不再直接显示乱码
- 🐛 **.env 显式 LLM / 搜索配置优先于残留环境变量**
  - 修复本地运行时 shell 中旧 `OPENAI_API_KEY` / `TAVILY_API_KEYS` 覆盖 `.env` 的问题
  - 当 `.env` 中显式配置 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL`、`OPENAI_API_STYLE`、`GEMINI_API_KEY`、`ANTHROPIC_API_KEY`、`TAVILY_API_KEYS` 等值时，运行期优先使用 `.env`
  - 避免用户已更新 `.env`，但进程仍误用外部环境中的旧 Key，导致 `401 INVALID_API_KEY`
- 🐛 **新闻搜索新增 GDELT 公开回退源**
  - 未配置 `Bocha/Tavily/Brave/SerpAPI` 时，系统自动回退到免 API Key 的 GDELT 新闻源
  - 搜索服务 availability、配置警告与前端「相关资讯」空状态文案已同步修正
  - GDELT 公共接口内置 5 秒节流与 429 退避重试，避免多维搜索时连续命中公共限流
- 🐛 **回测 performance 接口空数据不再返回 404**
  - `/api/v1/backtest/performance` 与 `/api/v1/backtest/performance/{code}` 在无汇总数据时返回零值指标
  - 避免浏览器网络面板持续出现 404，前端可直接显示空状态
- 🐛 **KEY INSIGHTS 空白时回退为明确结论**
  - AI 网关返回 `INVALID_API_KEY`、错误 JSON 或 HTML challenge 时，不再误存为空摘要的“成功分析”
  - 历史详情接口会优先从 `raw_result/dashboard/context` 回推出可读结论，旧记录刷新后也不会只显示“暂无分析结论”
- 🐛 **OpenAI 兼容层新增 Responses API 协议支持**
  - 背景：部分第三方 GPT 网关仅支持 `/v1/responses`，拒绝传统 `/v1/chat/completions`
  - 修复：新增 `OPENAI_API_STYLE` 配置项，支持在 `chat_completions` 与 `responses` 间切换
  - 覆盖范围：普通个股分析、Agent 问股、图片识别三条 OpenAI 兼容调用链路
  - 文档同步：更新 `README.md` 与 `.env.example`，补充 responses-only 网关示例
- 🐛 **Web 服务启动自动构建前端静态资源**
