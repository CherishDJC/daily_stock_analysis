...
| `ADMIN_AUTH_ENABLED` | Web 登录总开关：设为 `true` 后，页面与 `/api/v1/*` 需要登录 | `false` |
| `ADMIN_AUTH_USERNAME` | 管理员用户名，默认 `admin` | `admin` |
| `ADMIN_AUTH_PASSWORD` | 管理员明文密码（推荐仅在本地或受控环境使用） | - |
| `ADMIN_AUTH_PASSWORD_HASH` | 管理员 PBKDF2 哈希密码，使用 `python -m src.auth print_password_hash` 生成 | - |
| `HUMAN_VERIFY_ENABLED` | 启用登录页真人验证 | `false` |
| `HUMAN_VERIFY_PROVIDER` | 真人验证提供方，当前支持 `turnstile` | `turnstile` |
